...
public class StartupScanService
{
    ...
    public Task StartScan()
    {
        return Task.Run(() =>
        {
            Log.Debug("Startup scan starting");
            ...
            int addedCount = 0;
            int removedCount = 0;
            Dictionary<string, TrackedFileModel> trackedFiles =
                _trackerService.TrackedFiles.ToDictionary(x => x.FullPath);
            ...
            Log.Debug(
                "Startup Scan has found {scannedCount} documents, added/updated {addedCount} and removed {removedCount} files that no longer exist",
                scannedCount, addedCount, removedCount);
        });
    }
}